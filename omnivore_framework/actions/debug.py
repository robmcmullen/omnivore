...
class debug_text_counting(OmnivoreAction):
...
class debug_text_last_digit(OmnivoreActionRadioMixin, OmnivoreAction):
    def calc_name(self, action_key):
        return action_key.replace("_", " ").title()
...
class debug_text_last_digit_dyn(OmnivoreAction):
    def init_from_editor(self):
...
class debug_text_size(OmnivoreAction):
    def init_from_editor(self):
        self.counts = list(range(5, 25, 5))
