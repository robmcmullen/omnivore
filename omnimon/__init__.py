...
__url__ = "https://github.com/robmcmullen/omnimon"
__download_url__ = "https://github.com/robmcmullen/omnimon/releases"
__bug_report_url__ = "https://github.com/robmcmullen/omnimon/issues"
...
def get_py2exe_toolkit_includes(module=None, toolkit="wx"):
    ...
    setup(options={"py2exe": {"includes": omnimon.get_py2exe_toolkit_includes()}})
    """
    if module is None:
        path = __file__
    else:
...
def get_image_path(rel_path, module=None, file=None, up_one_level=False, excludes=[]):
    ...
    For example, in omnimon, the images are located in a directory "icons" in
    main omnimon directory (e.g. omnimon/icons):
        
    import omnimon
    image_path = get_image_path("icons", omnimon)
    
    will return the absolute path of "omnimon/icons".
    
    An example using the file keyword: if the current module is in the
    omnimon/framework directory, then the call to:
    
    get_image_path("icons", file=__file__)
    
    will contain the absolute path to the omnimon/framework/icons directory.
    """
    if file is None:
        if module is None:
            path = __file__
