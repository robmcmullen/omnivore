...
class FrameworkApplication(TasksApplication):
    ...
    id = 'omnimon.framework.application'
    ...
    startup_task = Str('omnimon.framework.text_edit_task')
    ...
    def _window_opened_fired(self, event):
        ...
        # memento (~/.config/Omnimon/tasks/wx/application_memento), the window
    ...
    def _initialize_application_home(self):
        ...
        from omnimon.third_party.appdirs import user_config_dir, user_log_dir, user_cache_dir
...
def run(plugins=[], use_eggs=True, egg_path=[], image_path=[], startup_task="", application_name="", debug_log=False):
    ...
    # Add omnimon icons after all image paths to allow user icon themes to take
    # precidence
    ...
    # Turn off omnimon log debug messages by default
    log = logging.getLogger("omnimon")
