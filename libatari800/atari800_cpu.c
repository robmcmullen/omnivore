...
#undef MEMORY_SetRAM
#undef MEMORY_SetROM
#undef MEMORY_SetHARDWARE

#define MEMORY_GetByte(addr)		(MEMORY_attrib[addr] == MEMORY_HARDWARE ? MEMORY_dHwGetByte(addr) : MEMORY_dGetByte(addr))
/* Reads a byte from ADDR, but without any side effects. */
#define MEMORY_SafeGetByte(addr)		(MEMORY_attrib[addr] == MEMORY_HARDWARE ? MEMORY_dSafeHwGetByte(addr) : MEMORY_dGetByte(addr))
#define MEMORY_PutByte(addr, byte)	 do { if (MEMORY_attrib[addr] == MEMORY_RAM) MEMORY_dPutByte(addr, byte); else if (MEMORY_attrib[addr] == MEMORY_HARDWARE) MEMORY_dHwPutByte(addr, byte); } while (0)
...
recreate_history_entry:
	...
	breakpoint_return:
		...
		if (entry) {
			...
			entry->num_bytes = instruction_length_6502[insn];
