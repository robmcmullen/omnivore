...
from omnivore8bit.arch.colors import powerup_colors

from . import SegmentViewer
from .jumpman_commands import *
from .jumpman_mouse_modes import *
...
#class MainFontMapScroller(FontMapScroller):
    ...
    def __init__(self, parent, linked_base, *args, **kwargs):
        BitmapScroller.__init__(self, parent, linked_base, rect_select=True, *args, **kwargs)
    ...
    def is_ready_to_render(self):
        return self.linked_base.editor is not None and self.level_builder is not None and self.segment is not None
    ...
    def get_segment(self):
        self.level_builder = JumpmanLevelBuilder(self.linked_base.editor.document.user_segments)
        self.trigger_root = None
        self.pick_buffer = self.segment_viewer.pick_buffer
        self.force_refresh = True
        self.generate_display_objects()
        return self.segment_viewer.screen

    def generate_display_objects(self, resync=False):
        try:
            source, level_addr, harvest_addr = self.segment_viewer.get_level_addrs()
            ...
            if self.trigger_root is not None:
                ...
                self.segment_viewer.trigger_list.recalc_view()
            if resync:
    ...
    def clear_screen(self):
        self.segment_viewer.clear_playfield()
        self.pick_buffer[:] = -1
    ...
    def redraw_current(self, screen, overlay_objects=[]):
        self.segment_viewer.clear_playfield(screen)
        self.pick_buffer[:] = -1
        self.level_builder.set_harvest_offset(self.mouse_mode.get_harvest_offset())
        main_state = self.level_builder.draw_objects(screen, None, self.linked_base.segment, highlight=overlay_objects, pick_buffer=self.pick_buffer)
        ...
        if self.trigger_root is not None:
            ...
            self.level_builder.draw_objects(screen, root, self.linked_base.segment, highlight=root, pick_buffer=self.pick_buffer)
            ...
            self.pick_buffer[:] = -1
            trigger_state = self.level_builder.draw_objects(screen, self.trigger_root.trigger_painting, self.linked_base.segment, highlight=overlay_objects, pick_buffer=self.pick_buffer)
    ...
    def bad_image(self):
        ...
        self.force_refresh = True
        s = self.segment.style.reshape((self.segment_viewer.antic_lines, -1))
    ...
    def get_image(self):
        ...
        if self.valid_level:
            ...
            return bitimage
        self.mouse_mode.draw_extra_objects(self.level_builder, self.segment, self.segment_viewer.segment)
        bitimage = BitmapScroller.get_image(self)
    ...
    def save_changes(self, command_cls=MoveObjectCommand):
        source, level_addr, old_harvest_addr = self.segment_viewer.get_level_addrs()
...
class JumpmanViewer(SegmentViewer):
    name = "jumpman"
    ...
    ##### Stuff that hasn't been migrated yet
